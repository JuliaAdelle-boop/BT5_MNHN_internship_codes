...
Resp_Matrix <- See %>%
  ...
  summarise(
    Gene_Expression_Index = mean(Gene_Expression_Index),
    .groups = 'keep')

Resp_Matrix <- as.data.table(Resp_Matrix)
pivot_Resp_Matrix <- dcast(Resp_Matrix, Station ~ CC, value.var = "Gene_Expression_Index", fill = 0)
...
print(Depth_values) #only 2 possible values (as expected)
...
#### The RDA (on Hellinger-transformed data)
...
png("RDA_Hellinger_UNKNOWNS_GGMM_GeneExpressionIndex.png", width = 800, height = 1000)
...
#can do ANOVA with anova.cca() to further analyse my results (according to some dude on the internet :))
