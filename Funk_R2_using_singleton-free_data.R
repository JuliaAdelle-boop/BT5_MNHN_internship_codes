...
CC_Abund_80=read.table('SMAGs_CC_Abund_80_all_noOLD_SAGs_singleton_free', header=F)
...
# print(sum(CC_Annot$CC_ID %in% "CC_10")) (just to check on a few ccs that what I got in cc_size was indeed the number of occurrences of each cc in CC_Annot)
...
#### Taxonomical indices ####
cc_taxo=read.table('CC_80_taxo_singleton_free', header=T, sep=" ")
...
func_homscore = function(x) {
  ...
  if(length(annot) == 0) {
    ...
    homscore = 1 - length(annot)/nprot  #same problem as in the previous func_homscore  
  }
  return(homscore)
}
...
write.table(cc_stats, "80CC_stats_with_taxo_singleton-free", quote = FALSE)
